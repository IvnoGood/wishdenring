...
def update():
    ...
    origin = player.world_position + Vec3(0, 1.5, 0)  # hauteur yeux
    if move != Vec3(0, 0, 0):
        hit = raycast(origin, move.normalized(), ignore=[player], distance=0.6)
        if not hit.hit:
            player.position += move

    # --- COLLISION AVEC LE SOL ---
    down_origin = player.world_position + Vec3(0, 1, 0)
    ground = raycast(down_origin, Vec3(0, -1, 0), ignore=[player], distance=3)

    if ground.hit:
        target_y = ground.world_point.y + player.height * 0.5
        player.y = lerp(player.y, target_y, 10 * time.dt)
    else:
